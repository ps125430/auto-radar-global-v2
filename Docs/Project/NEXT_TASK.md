...
- Never maintain two Current Tasks.
- `Next Task` is not authorization to start.
- Chat requests do not become active tasks until written here or explicitly approved for immediate implementation and then recorded here.
- When a task closes, update Current Task, Next Task, engineering or research status, and the product changelog in the same Project Sync.
